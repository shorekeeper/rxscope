...
//!   3. catalogue, because every widget resolves its wording through it;
//!   4. window, because the Vulkan surface needs a window handle;
//!   5. application loop.
...
fn main() {
    // A positional argument overrides the configuration path, which is what
    // lets several training profiles run side by side: a beginner set and a
    // contest speed set are two files rather than two installations.
    ...
    // The settings have to be readable before the logger is fully configured,
    // so the logger starts with defaults and is reconfigured immediately after.
    ...
    log_info!("main", "CWDrill {} starting", env!("CARGO_PKG_VERSION"));
    log_info!("main", "config: {}", config_path.display());

    // Written back at once so keys added by a newer build materialize and the
    // reference file stays complete for the operator.
    ...
    let exit_code = match app::App::new(settings) {
        Ok(mut a) => {
            let code = a.run();
            // Geometry and the training level change while the application
            // runs, so the settings are taken back rather than reused.
        ...
        Err(e) => {
            ...
            platform::message_box("CWDrill startup error", &format!("{}", e));
            1
        }
    };
