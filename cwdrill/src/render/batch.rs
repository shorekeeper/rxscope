...
pub enum Mode {
    ...
    Rgba = 2,
    Luma = 3,
    /// Single channel mapped through the palette texture.
    ///
    /// For data whose value spans orders of magnitude, where a brightness shows
    /// only the top of the range. The mapping happens in the shader, which is
    /// what lets the gamma move without rewriting the texture.
    Palette = 4,
}
...
pub struct DrawList {
    ...
    surface: Rect,
    /// Palette mapping, written into the push constants.
    ///
    /// Frame state rather than per command state, because one surface uses it and
    /// is drawn once. Carrying it per command would put three floats in every
    /// draw command to describe something only one of them reads.
    ///
    /// The stored byte, scaled by the first, divided by the second and raised to
    /// the third, indexes the palette.
...
impl DrawList {
    ...
    pub fn begin(&mut self, width: f32, height: f32, white: TextureId) {
        ...
        // before a consumer has stated its own does not divide by nought.
        self.palette_scale = 1.0;
        self.palette_span = 1.0;
        self.palette_gamma = 1.0;
    }
